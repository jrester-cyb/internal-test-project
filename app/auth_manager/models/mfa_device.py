...
class EmailDevice(MFADevice):
    """
    Email-based MFA device.
    Codes are sent via email to the configured address.
    """

    email = models.EmailField(
        blank=True,
        help_text="Email address for delivery (defaults to user's email if blank)",
    )
    ...
    def save(self, *args, **kwargs):
        if not self.name:
            self.name = f"Email ({self.get_masked_destination()})"
        super().save(*args, **kwargs)
    ...
    def delivery_email(self) -> str:
        """Get the email address to use for delivery."""
        return self.email or self.user.email

    def get_masked_destination(self) -> str | None:
        """Show first 2 chars and domain: ja***@example.com"""
    ...
    def send_code(self) -> str:
        """Generate and send a TOTP code via email."""
        code = self.generate_totp()
        self._send_email(code)
        return code

    def _send_email(self, code: str):
        ...
        send_mail(
            ...
            recipient_list=[self.delivery_email],
            fail_silently=False,
        )
