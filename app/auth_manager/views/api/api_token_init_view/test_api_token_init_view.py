...
class APITokenInitViewTests(TestCase):
    ...
    def test_invalid_token_string(self):
        response = self.client.post(self.url, {"token": "notavalidtoken"}, content_type="application/json")
    ...
    def test_expired_token(self):
        ...
        with mock.patch("django.conf.settings.SHORT_TOKEN_EXPIRATION", 0):
            response = self.client.post(self.url, {"token": token}, content_type="application/json")
            self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_token_with_invalid_user(self):
        # Generate a token for a non-existent OneTimeToken global_id
        fake_code = str(uuid4())
        # django
    ...
    def test__attempting_to_reuse_token_returns_expected_error(self):
        ...
        self.assertIn("token", response.json())
        self.assertTrue(AuthToken.objects.filter(user=self.user).exists())
        response_2 = self.client.post(self.url, {"token": token}, content_type="application/json")
