...
class SAMLIdentityProviderAuthenticationCallbackViewTestCase(TestCase):
    ...
    def test__get_returns_redirect_link_to_user(self):
        ...
        with patch(
            "auth_manager.models.identity_provider_models.reverse",
            return_value="http://testserver.com/saml/acs",
        ), patch(
            "onelogin.saml2.auth.OneLogin_Saml2_Auth.login",
            return_value="http://redirected.url",
        ):
        ...
        self.assertRedirects(
            response, "http://redirected.url", fetch_redirect_response=False
        )
    ...
    def test__post_returns_redirect_to_mfa_when_user_finalizes_their_mfa(self):
        ...
        self.saml_idp.sso_url = "http://example.com/saml/sso"
        self.saml_idp.x509_cert = """-----BEGIN CERTIFICATE-----"""
        self.saml_idp.attribute_mappings = {
            "email": "email",
            "first_name": "first_name",
            "last_name": "last_name",
        }
        self.saml_idp.save(
            update_fields=["entity_id", "sso_url", "x509_cert", "attribute_mappings"]
        )
        ...
        with patch(
            ...
            "onelogin.saml2.auth.OneLogin_Saml2_Auth.process_response",
            return_value=None,
        ), patch(
            "onelogin.saml2.auth.OneLogin_Saml2_Auth.get_errors", return_value=None
        ), patch(
            "onelogin.saml2.auth.OneLogin_Saml2_Auth.is_authenticated",
            return_value=True,
        ), patch(
        ...
        # assert
        self.assertRedirects(
            response, reverse("auth-manager:mfa"), fetch_redirect_response=False
        )

    def test__post_shows_error_when_user_does_not_exist_and_not_allowed_to_auto_enroll(
        self,
    ):
        # assign
        self.saml_idp.entity_id = "urn:powerview:myidp"
    ...
    def test__post_shows_error_when_user_already_linked_to_another_idp(self):
        ...
        self.assertContains(
            response,
            "User already linked to another identity provider",
            status_code=400,
        )
    ...
    def test__post_shows_error_when_saml_response_missing_required_attribute(self):
        ...
        # assert
        self.assertContains(
            response, "A valid email address is required.", status_code=400
        )
    ...
    def test__response_from_idp_with_email_that_does_not_match_provided_email_in_session_raises_error(
        self,
    ):
        # assign
        self.saml_idp.entity_id = "urn:powerview:myidp"
