...
class LocalIdentityProviderAuthenticationCallbackViewTestCase(TestCase):
    ...
    def test__shows_local_idp_page_when_idp_is_localidentityprovider(self):
        ...
        response = self.client.get(
            reverse("auth-manager:auth-callback", args=(self.local_idp.id,))
        )

        # assert
        self.assertContains(response, "Login", status_code=200)
        self.assertTemplateUsed(
            response, "idp_provider_templates/local_idp_provider.html"
        )
    ...
    def test__shows_locked_account_msg_when_user_exceeds_failed_login_attempts(
