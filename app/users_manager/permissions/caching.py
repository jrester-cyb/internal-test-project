...
class PermissionCache:
    ...
    def get_user_organization_ids(self, user: 'User') -> set[str]:
        ...
        if user.is_superuser:
            ...
            return set(str(oid) for oid in Organization.objects.values_list('id', flat=True))
    ...
    def get_user_workspace_ids(self, user: 'User') -> set[str]:
        ...
        if user.is_superuser:
            ...
            return set(str(wid) for wid in Workspace.objects.values_list('id', flat=True))
        ...
        # Direct memberships
        direct = WorkspaceMember.objects.filter(
            user=user
    ...
    def invalidate_role(self, role_id):
        ...
        from users_manager.models import (
            OrganizationMember, OrganizationGroupMember,
            WorkspaceMember, WorkspaceGroupMember,
            GroupMembership,
        )
        ...
        ws_groups = WorkspaceGroupMember.objects.filter(
            role_id=role_id
        ).values_list('group_id', flat=True)
...
class RequestPermissionCache:
    ...
    def get_organization_permissions(self, user, organization) -> set[str]:
        ...
        if org_id not in self.org_permissions:
            self.org_permissions[org_id] = permission_cache.get_organization_permissions(
                user, organization
            )
    ...
    def get_workspace_permissions(self, user, workspace) -> set[str]:
        ...
        if ws_id not in self.workspace_permissions:
            self.workspace_permissions[ws_id] = permission_cache.get_workspace_permissions(
                user, workspace
            )
