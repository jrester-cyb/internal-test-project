...
attr = AssetTypeAttribute.objects.create(
    asset_type=at, name="Name", api_key="name", attribute_type="text", order=1
)
asset = Asset.objects.create(asset_type=at, name="Building 1")

# Test single delete
val1 = TextAttributeValue.objects.create(
    asset=asset, attribute_type_attribute=attr, value="Value 1"
)
...
print(
    f"Exists in all_objects: {TextAttributeValue.all_objects.filter(id=val1.id).exists()}"
)
retrieved = TextAttributeValue.all_objects.get(id=val1.id)
print(f"deleted_at: {retrieved.deleted_at}")

# Test bulk delete
val2 = TextAttributeValue.objects.create(
    asset=asset, attribute_type_attribute=attr, value="Value 2"
)
print(f"\nCreated val2 with same asset/attribute (replaces deleted val1)")

# Create another asset attribute for bulk delete test
attr2 = AssetTypeAttribute.objects.create(
    asset_type=at,
    name="Description",
    api_key="description",
    attribute_type="text",
...
val3 = TextAttributeValue.objects.create(
    asset=asset, attribute_type_attribute=attr2, value="Value 3"
)
